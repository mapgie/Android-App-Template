...
Run the "Release" GitHub Actions workflow (`workflow_dispatch`). It consolidates
all fragments in this directory into a single new `CHANGELOG.md` entry, bumps
`versionCode` and `versionName` in `app/build.gradle.kts` accordingly, removes
the consumed fragments, and pushes directly to `main`.
